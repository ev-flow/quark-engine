...
version: 1.0.0
allowed-tools: bash(.claude/skills/quark/scripts/run_analysis.sh)
---
...
```bash
bash .claude/skills/quark/scripts/run_analysis.sh "<apk_path>" <flags>
```
...
```bash
# Summary report
bash .claude/skills/quark/scripts/run_analysis.sh "sample.apk" -s

# JSON report to file
bash .claude/skills/quark/scripts/run_analysis.sh "sample.apk" -o report.json

# Call graph PNG (requires -s)
bash .claude/skills/quark/scripts/run_analysis.sh "sample.apk" -s -g png

# List native APIs only (no rules needed)
bash .claude/skills/quark/scripts/run_analysis.sh "sample.apk" -i native

# Summary with a custom rules directory
bash .claude/skills/quark/scripts/run_analysis.sh "sample.apk" -s -r ~/my-rules/

# Run a single rule file
bash .claude/skills/quark/scripts/run_analysis.sh "sample.apk" -s path/to/rule.json
```
